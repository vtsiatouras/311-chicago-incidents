  ...
  <div class="container">
    ...
    <form name="form" @submit.prevent="handleSubmit">
      <div v-if="!successful">
        ...
        <div
            class="form-group"
            v-if="incident.type_of_service_request === 'RODENT_BAITING'"
        >
          <h4>Rodent Baiting</h4>
          <label for="number_of_premises_baited">Number of Premises Baited</label>
          <b-form-input
              class="mb-3"
              v-model="rodentBaiting.number_of_premises_baited"
              placeholder="Enter the number of baited premises"
              type="number"
          >
          </b-form-input>
          <div
              v-if="submitted && errors.has('number_of_premises_baited')"
              class="alert-danger"
          >
            {{ errors.first("number_of_premises_baited") }}
          </div>
          <label for="number_of_premises_w_garbage">Number of Premises With Garbage</label>
          <b-form-input
              class="mb-3"
              v-model="rodentBaiting.number_of_premises_w_garbage"
              placeholder="Enter the number of premises with garbage"
              type="number"
          >
          </b-form-input>
          <div
              v-if="submitted && errors.has('number_of_premises_w_garbage')"
              class="alert-danger"
          >
            {{ errors.first("number_of_premises_w_garbage") }}
          </div>
          <label for="number_of_premises_w_rats">Number of Premises With Rats</label>
          <b-form-input
              class="mb-3"
              v-model="rodentBaiting.number_of_premises_w_rats"
              placeholder="Enter the number of premises with rats"
              type="number"
          >
          </b-form-input>
          <div
              v-if="submitted && errors.has('number_of_premises_w_rats')"
              class="alert-danger"
          >
            {{ errors.first("number_of_premises_w_rats") }}
          </div>
        </div>
        <div
            class="form-group"
  ...
  methods: {
    ...
    serviceDispatcher() {
      if (this.incident.type_of_service_request === "ABANDONED_VEHICLE") {
        ...
        return CreateIncidentService.graffitiIncident(this.incident, this.graffiti);
      } else if (this.incident.type_of_service_request === 'RODENT_BAITING') {
        return CreateIncidentService.rodentBaitingIncident(this.incident, this.incidentActivity, this.rodentBaiting);
      } else if (this.incident.type_of_service_request === 'SANITATION_CODE') {
        return CreateIncidentService.sanitationCodeIncident(this.incident, this.sanitationCode);
      } else if (['TREE_TRIM', 'TREE_DEBRIS'].includes(this.incident.type_of_service_request)) {
        return CreateIncidentService.treeIncident(this.incident, this.incidentActivity, this.tree);
      } else {
