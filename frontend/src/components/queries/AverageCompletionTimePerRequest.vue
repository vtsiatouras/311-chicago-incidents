<template>
  <div class="container">
    <header class="jumbotron">
      <h3>Average Completion Time per Request</h3>
      <hr class="my-4"/>
      <p>
    ...
    <div v-if="message && successful">
      <table v-if="message" class="table">
        ...
        <tr v-for="request in message" v-bind:key="request.id">
          ...
          <td>{{ request.average_completion_time }}</td>
        </tr>
      </table>
    </div>
  </div>
...
export default {
  name: "AverageCompletionTimePerRequest",
  data() {
    return {
      startDate: '',
      endDate: '',
  ...
  methods: {
    handleSubmit() {
      ...
      this.$validator.validate().then((isValid) => {
        if (isValid) {
          QueriesService.averageCompletionTimePerRequest(this.startDate, this.endDate).then(
              (response) => {
