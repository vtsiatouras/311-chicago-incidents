...
class IncidentTests(BaseAPITestCase):
    ...
    def test_incident_create(self):
        ...
        data = {'creation_date': '2020-11-15T23:11:07.285Z', 'completion_date': '2020-11-15T23:11:07.285Z',
                'status': 'OPEN', 'service_request_number': '987654-qwe',
                'type_of_service_request': 'ABANDONED_VEHICLE', 'street_address': 'Test Address 123', 'zip_code': 0,
                'zip_codes': 0, 'x_coordinate': 12.134, 'y_coordinate': 12.134, 'latitude': 12.134, 'longitude': 12.134,
                'ward': 0, 'wards': 0, 'historical_wards_03_15': 0, 'police_district': 0, 'community_area': 0,
                'community_areas': 0, 'ssa': 0, 'census_tracts': 0}
    ...
    def test_incident_create_malformed_values(self):
        """Test that user can't input malformed values
        """
        self.authenticate('user')

        data = {'creation_date': '2020-11-15T23:11:07.285Z', 'completion_date': '2020-11-15T23:11:07.285Z',
                'status': 'asdf', 'service_request_number': '987654-qwe',
                'type_of_service_request': 'ABANDONED_VEHICLE', 'street_address': 'Test Address 123', 'zip_code': 0,
                'zip_codes': 0, 'x_coordinate': 12.134, 'y_coordinate': 12.134, 'latitude': 12.134, 'longitude': 12.134,
                'ward': 0, 'wards': 0, 'historical_wards_03_15': 0, 'police_district': 0, 'community_area': 0,
                'community_areas': 0, 'ssa': 0, 'census_tracts': 0}
    ...
    def test_abandoned_vehicle_incident_create(self):
        ...
        response = self.client.post(reverse('incident-abandoned-vehicle-incident'), data=data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        # Make sure that we didn't create duplicate abandoned vehicle
    ...
    def test_potholes_and_carts_incident_create(self):
        ...
        response = self.client.post(reverse('incident-garbage-carts-and-potholes-incident'), data=data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        # Check that we can create incident without carts-and-potholes & activity
    ...
    def test_potholes_and_carts_incident_create_malformed_values(self):
        """Test that user can't input malformed values
        """
        self.authenticate('user')
